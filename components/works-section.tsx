...
function ArticleCard({ blog, delay, isVisible, className = "" }: ArticleCardProps) {
      ...
      <div className="relative rounded-2xl overflow-hidden h-96 md:h-[600px]">
        <img
          ...
          className={`w-full h-full object-cover transition-all duration-500 ${
            isHovered && !isAdmin ? "blur-[2px]" : ""
          }`}
        />
        
        {/* Reduced glass effect overlay - only on main page */}
        {!isAdmin && (
          <div
            className={`absolute inset-0 bg-black/15 backdrop-blur-sm transition-opacity duration-500 ${
        ...
        {/* READ FULL ARTICLE text that follows mouse - only on main page */}
        {!isAdmin && blog.slug && (
          <Link
            href={`/blog/${blog.slug}`}
            className={`absolute transition-opacity duration-300 cursor-pointer z-50 ${
          ...
          >
            {/* Single glass effect circular overlay */}
            <div 
              className="bg-black/30 backdrop-blur-md rounded-full flex items-center justify-center"
              style={{ 
                width: "220px", 
                height: "220px"
              }} 
            >
              ...
              <div className="text-center">
                <span className="text-white font-light text-lg md:text-xl uppercase tracking-wider block leading-tight whitespace-pre-line">
...
function SmallArticleCard({ blog, delay, isVisible }: ArticleCardProps) {
  const [isHovered, setIsHovered] = useState(false)
  const [mousePosition, setMousePosition] = useState({ x: 0, y: 0 })
  const cardRef = useRef<HTMLDivElement>(null)
  const pathname = usePathname()
        ...
        {!isAdmin && blog.slug && (
          ...
          >
            ...
            >
              {/* Text inside the circle */}
              <div className="text-center">
                <span className="text-white font-light text-sm md:text-base uppercase tracking-wider block leading-tight whitespace-pre-line">
                  {t("works.readFullArticle")}
                </span>
              </div>
            </div>
